...
for g_letter, group_teams in groups.items():
    ...
    third_data = sorted_standings[2][1]
    thirds_stats.append((
        sorted_standings[2][0],                 # team name
        third_data['pts'],                       # points
        third_data['gf'] - third_data['ga'],     # goal difference
        third_data['gf'],                        # goals for
        g_letter
    ))
...
# =====================================================================
# Manual tiebreaker for teams that are completely level on pts / GD / GF.
# FIFA then uses fair-play points and the FIFA ranking, which we don't have
# in the dataset — so ties are broken explicitly here.
# Higher value = ranked higher.
manual_third_tiebreak = {
    'Ghana': 1,     # Ghana placed above Ecuador among level thirds
    'Ecuador': 0,
}

best_thirds_sorted = sorted(
    thirds_stats,
    key=lambda x: (x[1], x[2], x[3], manual_third_tiebreak.get(x[0], 0.5)),
    reverse=True
)
...
print("╚" + "═"*70 + "╝")
print("  ┌" + "─"*32 + "┬" + "─"*7 + "┬" + "─"*7 + "┬" + "─"*7 + "┬" + "─"*16 + "┐")
print("  │ " + "Team".ljust(30) + " │ " + "Grp".center(5) + " │ " + "Pts".center(5) + " │ " + "GD".center(5) + " │ " + "Status".center(14) + " │")
print("  ├" + "─"*32 + "┼" + "─"*7 + "┼" + "─"*7 + "┼" + "─"*7 + "┼" + "─"*16 + "┤")
for idx, (team, pts, gd, gf, group) in enumerate(best_thirds_sorted, 1):
    status       = "QUALIFIED" if idx <= 8 else "ELIMINATED"
    team_display = f"{idx}. {team}"
    gd_str       = f"+{gd}" if gd > 0 else str(gd)
    print(f"  │ {team_display:<30} │ {group:^5} │ {pts:^5} │ {gd_str:^5} │ {status:^14} │")
    if idx == 8:
        print("  ├" + "─"*32 + "┼" + "─"*7 + "┼" + "─"*7 + "┼" + "─"*7 + "┼" + "─"*16 + "┤")
print("  └" + "─"*32 + "┴" + "─"*7 + "┴" + "─"*7 + "┴" + "─"*7 + "┴" + "─"*16 + "┘")
...
qualified_map = {group: team for team, pts, gd, gf, group in qualified_thirds}
...
# Match 87 : Winner K     vs 3rd-place from L
